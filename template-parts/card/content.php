...
<article <?php post_class( $article_class ); ?> aria-labelledby="post-<?php the_ID(); ?>">
	...
	<div class="card__wrapper p-4 flex-1">
		<header class="card__header mb-4">
			<div class="flex flex-wrap alignwide items-center">
				<?php
				// Display post type name in search results.
				if ( is_search() && $posttype ) {
					echo '<span class="card__posttype text-sm">' . esc_html( $posttype->labels->singular_name ) . '</span>';
				}
				...
				if ( has_category() ) {
					...
					echo sprintf(
						...
						( is_search() && $posttype ) ? '<span class="mx-2">|</span>' : '', // phpcs:ignore.
						sprintf(
